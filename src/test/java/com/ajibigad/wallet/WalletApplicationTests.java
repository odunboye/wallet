...
@SpringBootTest
@ContextConfiguration(initializers = {WalletApplicationTests.Initializer.class})
public class WalletApplicationTests {
	...
	public void testWallet() {
		...
		// Simulate several request to an account with random amounts and sometimes credit, sometimes debit
		// Fetch the wallet balance from wallet table
		// Fetch sum of all transactions for the test account and assert the result is equal to the balance in the wallet
		...
		random.longs(200, 1, 10000)
				.forEach(l -> {
					...
					tasks.add(() -> {
						walletService.processTransactions(generateTransactionsForDebit(BigDecimalUtils.multiply(0.02, amount)));
						return null;
					});
				});
		try {
		...
		System.out.println("Total transactions created -> " + walletTransactionRepository.count());
		...
		BigDecimal merchantBalance = walletTransactionRepository.getRolledUpBalance(MERCHANT_ACCT);
		BigDecimal providerBalance = walletTransactionRepository.getRolledUpBalance(PROVIDER_ACCT);
		BigDecimal payonusBalance = walletTransactionRepository.getRolledUpBalance(PAYONUS_ACCT);
		...
		System.out.println(merchantWallet.getBalance().toPlainString() + " -> " + merchantBalance.toPlainString());
		System.out.println(providerWallet.getBalance().toPlainString() + " -> " + providerBalance.toPlainString());
		System.out.println(payonusWallet.getBalance().toPlainString() + " -> " + payonusBalance.toPlainString());

		//View loss or gain due to rounding
		BigDecimal totalAmount = BigDecimal.valueOf(totalAmounts[0], 2);

		BigDecimal merchantBalanceWithHigherPrecision = BigDecimalUtils.multiply(0.75, totalAmount, false).subtract(BigDecimalUtils.multiply(0.02, totalAmount, false));
		BigDecimal providerBalanceWithHigherPrecision = BigDecimalUtils.multiply(0.05, totalAmount, false);
		BigDecimal payonusBalanceWithHigherPrecision = BigDecimalUtils.multiply(0.20, totalAmount, false).add(BigDecimalUtils.multiply(0.02, totalAmount));

		System.out.println(merchantBalanceWithHigherPrecision + " -> " + merchantBalance + ", Diff -> " + merchantBalance.subtract(merchantBalanceWithHigherPrecision));
		System.out.println(providerBalanceWithHigherPrecision + " -> " + providerBalance + ", Diff -> " + providerBalance.subtract(providerBalanceWithHigherPrecision));
		System.out.println(payonusBalanceWithHigherPrecision + " -> " + payonusBalance + ", Diff -> " + payonusBalance.subtract(payonusBalanceWithHigherPrecision));
	}

	private List<WalletTransaction> generateTransactionsForCredit(BigDecimal amount) {
	...
	private List<WalletTransaction> generateTransactionsForDebit(BigDecimal amount) {
		List<WalletTransaction> transactions = new ArrayList<>();
		WalletTransaction merchantCredit = WalletTransaction.builder()
				.accountNumber(MERCHANT_ACCT)
				.amount(amount)
